...
class LaneTracker(object):
    ...
    def __init__(self, first_frame, n_windows=15):
        """
    ...
    def process(self, frame, draw_lane=True, draw_statistics=True):
        ...
        if draw_statistics:
            ...
            frame[:200, :, :] = frame[:200, :, :] * .4
            (h, w, _) = debug_overlay.shape
            frame[20:20 + h, 20:20 + w, :] = debug_overlay
            frame[20:20 + h, 20 + 20 + w:20 + 20 + w + w, :] = top_overlay
            text_x = 20 + w + w + 20
            self.draw_text(frame, 'radius curve:  {} m'.format(self.radius_of_curvature()), text_x, 50)
            self.draw_text(frame, 'Distance from center (left):  {:.1f} m'.format(self.left.camera_distance()), text_x, 110)
            self.draw_text(frame, 'Distance from center (right):  {:.1f} m'.format(self.right.camera_distance()), text_x, 170)
